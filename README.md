...
4. `.kelpie/artifacts/.../issue-xx/` 配下に prompt キャッシュ、intent record、check ファイルを作る
...
- Git レイアウトを自動検出し、必要なら worktree ではなく repo root を `/workspace` に bind mount
- `run_issue_workflow.py --repo-root /opt/kelpie --workdir <container-path> ...` を実行

`--no-build` を付けると build を省略できます。

### Git mount strategy

`run_issue_workflow_in_container.sh` は `git rev-parse` を使って、通常の clone と worktree を判別します。

- `auto`
  デフォルト。`--target-workdir` が worktree なら、repo root を mount しつつ、コンテナ内 `--workdir` は worktree の相対パスへ合わせます
- `worktree-only`
  常に指定ディレクトリだけを mount します
- `repo-root`
  常に Git repo root を mount します。対象が Git 管理下である必要があります

コマンド引数でも指定できます。

```bash
kelpie --git-mount-strategy repo-root --target-workdir /path/to/worktree -- --issue 23 --runner codex
```

また、`~/.config/kelpie/runner.env` にデフォルトを書けます。

```bash
GIT_MOUNT_STRATEGY=auto
```
...
Issue コメントを含める場合、GitHub から取得した JSON は `.kelpie/artifacts/github/<owner>/<repo>/issue-xx/.issue-cache/` に保存されます。`--issue-source file` の場合は `.kelpie/artifacts/file/local/issue-xx/` を使います。
